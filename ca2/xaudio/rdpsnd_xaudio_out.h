...
#include "../channels/rdpsnd/client/rdpsnd_main.h"
//#include "../codec_aac.h"
...
typedef struct xaudio_out * HWAVEOUT;
...
public:
   ...
   virtual int out_open_ex(int32_t iBufferCount,int32_t iBufferSampleCount,uint32_t uiSamplesPerSec,uint32_t uiChannelCount,uint32_t uiBitsPerSample);
   virtual int out_stop();
   virtual int out_close();
   virtual int out_pause();
   virtual int out_restart();
